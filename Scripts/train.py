...
def get_model_by_keyboard_input() -> ClassifierModel:
    ...
    if selected_model == 'log_res':
        ...
        if selected_penalty == "none":
            selected_C ='inf'
        else:
            selected_C = ''
            while not is_non_negative_float(selected_C) and selected_C!="0":
                selected_C = input("Enter C, the inverse of regularization strenght (positive float): ").lower()
...
def main(keyboard_input: bool=True):
    ...
    while wrong_parameter(selected_evaluation, ['y', 'n']):
        selected_evaluation = input("Do you want to log the evaluation of the model? (Y/n) ").lower()
